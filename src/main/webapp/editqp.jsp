...
<body class="bg-light">
	...
	<nav aria-label="breadcrumb" id="breadcrumb" style="margin-left: 165px">
	  <ol class="breadcrumb">
	    ...
	    <li class="breadcrumb-item">Edit Price Quote</li>
	  </ol>
	</nav>
    ...
    <div class="bg-white rounded-0 mt-2 overflow-auto" id="constraint">
    	<div class="container-fluid text-center pt-4">
    		<div class="row d-flex align-items-center justify-content-center">
    			...
    			<div class="col" style="margin-right: 120px" id="progressed2">
    				<div class="d-flex align-items-center justify-content-center" style="color: #4D73FF">
    	...
    	<form id="PDform" action="EditCalc" method="get" onsubmit="return validate()">
    	<div class="row g-4 border-bottom border-2" style="margin-left: 25px; margin-right:5px">
    		...
    		<div class="col-md-3">
    			<div class="d-flex flex-column">
    				<label for="ppcm" class="control-label opacity-75">Picks Per Inches</label>
    				<div class="row">
    					<div class="col-4">
    	...
    	<div class="container m-4 h5" id="WW"><span>Add Yarns</span></div>
    	
	    <div class="container m-4 h5" id="wwbtncontainer">
   		...
   		<table class="table table-lg table-bordered" style="margin-left: 40px; width: 95%" id="wtable">
		...
		<div class="container-fluid text-center mx-auto" style="width: 180px; display: flex" id="saveblk">
			<button class="btn border-0 btn-lg rounded-0 mb-3" style="background-color: #4D73FF; color: white; width: 180px; margin-right: 50px !important" type="button" id="backbtn" onclick="back()">BACK</button>
   			<button class="btn border-0 btn-lg rounded-0 mb-3" style="background-color: #4D73FF; color: white; width: 180px" type="submit" id="savebtn">SAVE</button>
   			<input type="hidden" name="quoteToEdit" value="${quoteToEdit}"/>
   		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			document.getElementById("pmodel").dispatchEvent(new Event('input'));
			document.getElementById("ppunit").value = "${ppunit}";
			document.getElementById("ppunit").dispatchEvent(new Event('change'));
			document.getElementById("ppcm").value = "${ppcm}";
			document.getElementById("ppcm").dispatchEvent(new Event('input'));
			document.getElementById("totalWarp").value = "${totalWarp}";
			document.getElementById("totalWarp").dispatchEvent(new Event('input'));
			document.getElementById("date").value = "${date}";
			document.getElementById("date").dispatchEvent(new Event('input'));
			document.getElementById("memo").value = "${quoteMemo}";
			document.getElementById("memo").dispatchEvent(new Event('input'));
			document.getElementById("fabrictype").value = "${ftype}";
			document.getElementById("fabrictype").dispatchEvent(new Event('change'));
			
			var selectedFinStr = "${finishing}";
			const selectedFinArr = selectedFinStr.split(",");
			document.querySelector('#ppmodel').setValue(selectedFinArr, false);
			
			var selectedMatStr = "${selectedMaterials}";
			const selectedMatArr = selectedMatStr.split(":");
			for (var i=0; i<selectedMatArr.length-1; i++) {
				if (i != 0) {
					document.getElementById("WWbtn").click();
				}
				var individualMatName = "";
				var individualMatType = "";
				var individualMatColor = "";
				var individualMatMemo = "";
				var individualMatUsePercent = "";
				const individualMatArr = selectedMatArr[i].split(',');
				for (var j=0; j<individualMatArr.length-1; j++) {
					if (j == 0) {
						individualMatName = individualMatArr[j];
					} else if (j == 1) {
						individualMatType = individualMatArr[j];
					} else if (j == 2) {
						individualMatColor = individualMatArr[j];
					} else if (j == 3) {
						individualMatMemo = individualMatArr[j];
					} else {
						individualMatUsePercent = individualMatArr[j];
					}
				}
				var idx = i+1;
				document.getElementById("wwtype" + idx).value = individualMatType;
				document.getElementById("wwtype" + idx).dispatchEvent(new Event('change'));
				document.getElementById("usePercent" + idx).value = individualMatUsePercent;
				document.getElementById("usePercent" + idx).dispatchEvent(new Event('input'));
				document.getElementById("matMemo" + idx).value = individualMatMemo;
				document.getElementById("matMemo" + idx).dispatchEvent(new Event('input'));
				
				var $select = $('#mattype' + idx).selectize();
				var selectize = $select[0].selectize;
				selectize.setValue(individualMatName, false);
				
				document.getElementById("colortype" + idx).value = individualMatColor;
				document.getElementById("colortype" + idx).dispatchEvent(new Event('change'));
			}
		});
		...
		$( "#WWbtn" )
		  .on( "click", function() {
			...
			var costselector = "#matcost" + counter;
		...
		function validate() {
			if (document.forms["PDform"]["pmodel"].value == "") {
				...
				alert("Please enter picks per units!");
				return false;
				...
				for (var i=1; i<=counter; i++) {
					try {
						if (document.getElementById("wwtype" + i).value == "" || document.getElementById("wwtype" + i).value == "Enter") {
							...
							return false;
						} else if (document.getElementById("colortype" + i).value == "" || document.getElementById("colortype" + i).value == "Enter") {
