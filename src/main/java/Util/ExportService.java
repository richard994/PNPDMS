...
public class ExportService extends HttpServlet{
    ...
    private void exportToExcel(ArrayList<Developments> developments, OutputStream out) throws IOException {
        ...
        for (Developments dev : developments) {
    		...
    		row.createCell(5).setCellValue(dev.isSDY() ? "yes" : "no");
    		row.createCell(6).setCellValue(dev.isIs400hrFCL() ? "yes" : "no");
    		row.createCell(7).setCellValue(dev.isIs1000hrFCL() ? "yes" : "no");
    		row.createCell(8).setCellValue(dev.isPieceDyed() ? "yes" : "no");
    		row.createCell(9).setCellValue(dev.isNeedFeedback() ? "yes" : "no");
    		row.createCell(10).setCellValue(dev.isNeedChinaFeedback() ? "yes" : "no");
    		row.createCell(11).setCellValue(dev.isChenille() ? "yes" : "no");
    		row.createCell(12).setCellValue(dev.isKnit() ? "yes" : "no");
    		row.createCell(13).setCellValue(dev.isGeorgeCanceled() ? "yes" : "no");
    		row.createCell(14).setCellValue(dev.isInactive() ? "yes" : "no");
    		row.createCell(15).setCellValue(dev.getFabric_type());
    		row.createCell(16).setCellValue(dev.getDesign_type());
    		row.createCell(17).setCellValue(dev.getColorist());
    		row.createCell(18).setCellValue(dev.getDesigner());
    		row.createCell(19).setCellValue(dev.getDirection());
    		row.createCell(20).setCellValue(dev.getFinishing_used());
    		row.createCell(21).setCellValue(dev.getSeason());
    		row.createCell(22).setCellValue(dev.getYarn_type());
    		row.createCell(23).setCellValue(dev.getWarp_type());
    		row.createCell(24).setCellValue(dev.getContent());
