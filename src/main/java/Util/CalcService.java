...
public class CalcService extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse
		...
		for (int i=1; i<50; i++) {
			...
			System.out.println("type: " + mtrtype + ".");
			
			double hdr = 0;
			double cg = matData.getCountGu(buffer2);
			double cz = matData.getCountZhi(buffer2);
			
			System.out.println("Count zhi: " + cz + "; count gu: " + cg + ".");
			...
			if (buffer1.equals("Weft")) {
				...
				System.out.println("warp cost for material number " + i + " is " + warpCost);
			} else {
		...
		SalePrice = String.format("%.2f", finalSalePrice);
